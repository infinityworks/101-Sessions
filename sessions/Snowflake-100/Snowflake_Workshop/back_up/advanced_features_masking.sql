...
-- Excercise
-- Mask all PII data in the below table for the role ANALYST.
use role accountadmin;
...
-- Mask all PII data in the below table for the role ANALYST.
-- start here:
alter table  masking.masking.customer_exercise modify column c_first_name set masking policy masking.masking.hashing_for_analyst_role;
alter table  masking.masking.customer_exercise modify column c_last_name set masking policy masking.masking.hashing_for_analyst_role;
alter table  masking.masking.customer_exercise modify column c_email_Address set masking policy masking.masking.hashing_for_analyst_role;
...
create or replace row access policy masking.masking.ountry_level_access as (country varchar) returns boolean ->
   current_role()='ACCOUNTADMIN'
      or exists (
            select 1 from masking.masking.country_lookup
              where role = current_role()
                and country_name = country
          );


ALTER TABLE masking.masking.customer_exercise ADD ROW ACCESS POLICY masking.masking.ountry_level_access ON (c_birth_country);
...
-- Clean-Up
use role accountadmin;
drop schema if exists masking.masking;
drop role if exists ANALYST;
drop role if exists east_analyst;
drop role if exists west_analyst;
drop role if exists north_analyst;
drop role if exists south_analyst;
